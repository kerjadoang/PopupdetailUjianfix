...
const AKMMCQ: React.FC<MCQScreenProp> = props => {
  ...
  const onKumpulkan = async () => {
    try {
      const res = await provider.finishTest(
        question_data?.data?.package_history_id,
      );
      navigation.replace('ResultScreen', {
        ...
        duration: `${res?.data?.data?.duration ?? 0} menit`,
  ...
  const onPauseResume = async () => {
    try {
      if (!checkUnfinisedData?.data?.paused) {
        ...
        const res = await provider.resumeTest(
          question_data?.data?.package_history_id,
        );
        const timerEnd = new Date(res?.data?.data?.end_time || '').getTime();
...
export default React.memo(AKMMCQ);
