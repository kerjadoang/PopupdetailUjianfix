...
const PackageCoinSwipeUp: FC<Props> = ({
  selectedPackage,
  onPayment,
  promoStatus,
  promoVoucher,
  ...
  const currentPrice = promoStatus?.statusSuccess
    ? promoVoucher?.price_after_discount
    ...
    <MainView padding={16}>
      ...
      <MainView gap={8}>
        <MainView justifyContent="space-between" flexDirection="row">
          ...
          <MainText>{currencyFormat(selectedPackage?.price ?? 0)}</MainText>
        </MainView>
...
export default PackageCoinSwipeUp;
