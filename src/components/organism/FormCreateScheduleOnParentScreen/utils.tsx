...
const useAnnouncementManageCreateScreen = () => {
  ...
  const __handleSave = async () => {
    ...
    if (
      ...
      const user_type = receivers?.map((val: any) => {
        ...
        if (data?.class) {
          data = {
            ...
            name: data?.name,
            class: data?.class
              ?.filter((_val: any) => _val?.parent_id)
              .map(({parent_id: _, ...values}: any) => values),
          };
        }
      ...
      try {
        const {status} = await provider.postAnnouncement(body);

        if (status === 200) {
          Toast.show({
            type: 'success',
            text1: 'Pengumuman berhasil dibuat.',
          });
      ...
      if (datas.length > 0) {
        datas = datas.map((val: any) => {
          if (val?.class?.length > 0) {
            val.class = val?.class?.map((subVal: any) => ({
              ...subVal,
              parent_id: val?.id,
            }));
