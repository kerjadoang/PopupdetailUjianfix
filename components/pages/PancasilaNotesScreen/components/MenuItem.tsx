...
const MenuItem: React.FC<MenuItemProps> = props => {
  ...
  const isGuru = props.service_type === 'guru';
  const isKepsek = props.service_type === 'kepsek';
  return (
    <SwipeUp {...props}>
      <View style={{padding: 16, gap: 24}}>
        {menus.map((item, index) => {
          if ((isGuru || isKepsek) && item.title === 'Bagikan') {
            return <View key={index} />;
          }
          return (
            item.types.includes(props.type) && <Item key={index} {...item} />
          );
        })}
